...
export class Chart extends EventEmitter.EventEmitter {
  ...
  updateAxes() {
    ...
    if (this.options.x.position === 'sticky') {
      ...
      const textHeight = canvas.select('.x.axis text').node().getBBox().height
      const minTranslation = 0
      const maxTranslation = this.meta.height - textHeight - 3
      yTranslation = yTranslation < minTranslation ? minTranslation : yTranslation
      yTranslation = yTranslation > maxTranslation ? maxTranslation : yTranslation
    ...
    if (this.options.y.position === 'sticky') {
      ...
      const textWidth = canvas.select('.y.axis text').node().getBBox().width
      const minTranslation = textWidth + 3
      const maxTranslation = this.meta.width
      xTranslation = xTranslation < minTranslation ? minTranslation : xTranslation
      xTranslation = xTranslation > maxTranslation ? maxTranslation : xTranslation
